        ...
        if(segue.identifier == "createParty") {
            ...
            if useLocationSwitch.isOn {
                ...
                if( CLLocationManager.authorizationStatus() == .authorizedWhenInUse ||
                    ...
                    queue.restrict_location = true
                    queue.location_min = Double(locationRadiusInput.text!)!
                } else {
                    queue.restrict_location = false
                }
            } else {
                queue.restrict_location = false
            }
            
            queue.vote_threshold = voteThresholdSwitch.isOn ? Int(voteThresholdInput.text!)! : -1
            queue.allow_duplicated = duplicateSongsSwitch.isOn
            queue.open = true
